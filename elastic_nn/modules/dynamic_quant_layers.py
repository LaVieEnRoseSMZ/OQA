...
from elastic_nn.modules.lsq_layers import MBInvertedLSQConvLayer
from elastic_nn.modules.dynamic_quant_op import DynamicLSQPointConv2d, DynamicLSQSeparableConv2d
from elastic_nn.utils import adjust_bn_according_to_idx, copy_bn


class DynamicLSQMBConvLayer(MyModule):
    
    def __init__(self, in_channel_list, out_channel_list,
                 kernel_size_list=3, expand_ratio_list=6, stride=1, act_func='relu6', use_se=False, nbit_a=4, nbit_w=4):
        super(DynamicLSQMBConvLayer, self).__init__()
        ...
        if max(self.expand_ratio_list) == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                ('conv', DynamicLSQPointConv2d(max(self.in_channel_list), max_middle_channel, nbit_a=nbit_a, nbit_w=nbit_w)),
                ('bn', DynamicBatchNorm2d(max_middle_channel)),
                ('act', build_activation(self.act_func, inplace=True)),
            ]))
        if not self.use_se:
            self.depth_conv = nn.Sequential(OrderedDict([
                ('conv', DynamicLSQSeparableConv2d(max_middle_channel, self.kernel_size_list, self.stride, nbit_a=nbit_a, nbit_w=nbit_w)),
                ('bn', DynamicBatchNorm2d(max_middle_channel)),
                ('act', build_activation(self.act_func, inplace=True))
            ]))
        else:
            self.depth_conv = nn.Sequential(OrderedDict([
                ...
                ('bn', DynamicBatchNorm2d(max_middle_channel)),
                ('act', build_activation(self.act_func, inplace=True)),
                ('se', DynamicSE(max_middle_channel)),
                #('act', build_activation(self.act_func, inplace=True))
            ]))
        # if self.use_se:
        ...
        self.point_linear = nn.Sequential(OrderedDict([
            ('conv', DynamicLSQPointConv2d(max_middle_channel, max(self.out_channel_list), nbit_a=nbit_a, nbit_w=nbit_w)),
    ...
    def config(self):
        return {
            'name': DynamicLSQMBConvLayer.__name__,
    ...
    def build_from_config(config):
        return DynamicLSQMBConvLayer(**config)
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        # build the new layer
        sub_layer = MBInvertedLSQConvLayer(
            in_channel, self.active_out_channel, self.active_kernel_size, self.stride, self.active_expand_ratio,
            act_func=self.act_func, mid_channels=middle_channel, use_se=self.use_se, nbit_a = self.nbit_a, nbit_w=self.nbit_w,
        )
        ...
        if sub_layer.inverted_bottleneck is not None:
            ...
            sub_layer.inverted_bottleneck.conv.sa.data.copy_(self.inverted_bottleneck.conv.conv.sa.data)
            sub_layer.inverted_bottleneck.conv.sw.data.copy_(self.inverted_bottleneck.conv.conv.sw.data)
            copy_bn(sub_layer.inverted_bottleneck.bn, self.inverted_bottleneck.bn.bn)

        sub_layer.depth_conv.conv.weight.data.copy_(
            self.depth_conv.conv.get_active_filter(middle_channel, self.active_kernel_size).data
        )
        sub_layer.depth_conv.conv.sa.data.copy_(self.depth_conv.conv.conv.sa.data)
        sub_layer.depth_conv.conv.sw.data.copy_(self.depth_conv.conv.conv.sw.data)
        ...
        sub_layer.point_linear.conv.sa.data.copy_(self.point_linear.conv.conv.sa.data)
        sub_layer.point_linear.conv.sw.data.copy_(self.point_linear.conv.conv.sw.data)
