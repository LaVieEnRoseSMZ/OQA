...
class DynamicLSQSeparableConv2d(nn.Module):
    KERNEL_TRANSFORM_MODE = None  # None or 1
    
    def __init__(self, max_in_channels, kernel_size_list, stride=1, dilation=1, nbit_a=4, nbit_w=4):
        super(DynamicLSQSeparableConv2d, self).__init__()
        ...
        # )
        self.conv = LsqConv(self.max_in_channels, self.max_in_channels, max(self.kernel_size_list),
                            quan_name_w='lsq', quan_name_a='lsq', nbit_w=nbit_w, nbit_a=nbit_a,
                            stride=self.stride, groups=self.max_in_channels, bias=False,)
    ...
    def forward(self, x, kernel_size=None):
        ...
        y = F.conv2d(
            x_quant, w_quant, None, self.stride, padding, self.dilation, in_channel
        )
        return y


class DynamicLSQPointConv2d(nn.Module):
    
    def __init__(self, max_in_channels, max_out_channels, kernel_size=1, stride=1, dilation=1, nbit_a=4, nbit_w=4):
        super(DynamicLSQPointConv2d, self).__init__()
        ...
        # )
        self.conv = LsqConv(self.max_in_channels, self.max_out_channels, self.kernel_size,
                            quan_name_w='lsq', quan_name_a='lsq', nbit_w=nbit_w, nbit_a=nbit_a,
    ...
    def forward(self, x, out_channel=None):
        ...
        y = F.conv2d(x_quant, w_quant, None, self.stride, padding, self.dilation, 1)
        return y
