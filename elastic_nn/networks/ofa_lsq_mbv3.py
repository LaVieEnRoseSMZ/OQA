...
from elastic_nn.modules.dynamic_quant_layers import DynamicLSQMBConvLayer
from layers import ConvLayer, IdentityLayer, LinearLayer, MBInvertedConvLayer
from elastic_nn.modules.lsq_layers import MBInvertedLSQConvLayer, LSQConvLayer
...
class OFALSQMobileNetV3(MobileNetV3):

    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None,
                 width_mult_list=1.0, ks_list=3, expand_ratio_list=6, depth_list=4, bit_list=4, nbit_a=4, nbit_w=4):
        ...
        self.bit_list = int2list(bit_list, 1)
        ...
        if len(set(input_channel)) == 1:
            ...
            first_block_conv = MBInvertedLSQConvLayer(
                in_channels=max(input_channel), out_channels=max(input_channel), kernel_size=3, stride=stride_stages[0],
                expand_ratio=1, act_func=act_stages[0], use_se=se_stages[0], nbit_a=nbit_a, nbit_w=nbit_w,
            )
        else:
            ...
            first_block_conv = DynamicLSQMBConvLayer(
                in_channel_list=input_channel, out_channel_list=input_channel, kernel_size_list=3, expand_ratio_list=1,
                stride=stride_stages[0], act_func=act_stages[0], use_se=se_stages[0], nbit_a=nbit_a, nbit_w=nbit_w,
            )
        ...
        for width, n_block, s, act_func, use_se in zip(width_list[1:], n_block_list[1:],
            ...
            for i in range(n_block):
                if i == 0:
                    ...
                    stride = 1
                mobile_inverted_conv = DynamicLSQMBConvLayer(
        ...
        if len(final_expand_width) == 1:
            final_expand_layer = LSQConvLayer(
                max(feature_dim), max(final_expand_width), kernel_size=1, act_func='h_swish', 
                nbit_a = nbit_a, nbit_w=nbit_w)
            feature_mix_layer = LSQConvLayer(
        ...
        super(OFALSQMobileNetV3, self).__init__(first_conv, blocks, final_expand_layer, feature_mix_layer, classifier)
    ...
    def config(self):
        return {
            'name': OFALSQMobileNetV3.__name__,
