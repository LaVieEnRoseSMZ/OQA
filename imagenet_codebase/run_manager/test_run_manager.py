...
class TestRunConfig:
    ...
    def copy(self):
        return TestRunConfig(**self.config)
...
class TestRunManager:
    ...
    def __init__(self, path, net, run_config: TestRunConfig, init=True, measure_latency=None, no_gpu=False, mix_prec=None):
        self.path = path
        self.net = net
        ...
        self.flops = net_info['flops']
        self.params = net_info['params']
        #with open('%s/net_info.txt' % self.path, 'a') as fout:
        #    fout.write(json.dumps(net_info, indent=4) + '\n')
            #val_log='FLOPs: {0:.3f}\tparams: {1:.3f}'.\
            #        format(self.flops, self.params)
            #fout.write(val_log+'\n')
        #    try:
        ...
        with torch.no_grad():
            with tqdm(total=len(data_loader),
                ...
                for i, (images, labels) in enumerate(data_loader):
                    ...
                    t.update(1)
        return losses.avg, top1.avg, top5.avg, net
